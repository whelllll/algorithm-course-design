...
def evaluate(in_file, out_file):
    ...
    # E_memory: per-task GPU memory waste (NEW FORMULA 2026-06)
    # E_memory = sum_i p_i * (u_i * VG_{s_i} - v_i)
    # p_i = task duration / total duration (duration-weighted)
    ...
    for job in jobs:
        rec = records[job['id']]
        u_i = rec['gpu_used']
        VG_si = server_gpu_mem[rec['server']]
        v_i = job['gpu_memory']
        waste = u_i * VG_si - v_i
        if waste < 0:
            waste = 0.0
        p_i = job['duration'] / total_duration if total_duration > 0 else 1.0 / len(jobs)
        e_memory += p_i * waste

    return e_wait, e_memory, e_finish
...
if __name__ == '__main__':
    ...
    print(f"E_memory (单任务显存浪费): {e_memory:.2f}")
