...
| **v5.0** | **06-26** | **WSPT+时间感知** | **Best-Fit** | **112,236,844** | **40.15** | **22,498** | |
| **v5.1** | **06-26** | **WSPT+时间感知** | **显存浪费感知** | **112,237,548** | **27.87** | **22,490** | **← 当前** |
...
## v5.1 — 显存浪费感知选服务器（2026-06-26）← 当前

### 改了什么

选服务器逻辑从 Best-Fit（GPU剩余最少）改为**显存浪费感知**。

```
v5.0 Best-Fit: 选 remaining_gpu - gpu_used 最小的（GPU最紧凑）
v5.1 显存感知: 选 gpu_used × VG_si - job.gpu_memory 最小的（显存浪费最少）
             浪费相同时选GPU更紧凑的
```

### 原理

新E_memory公式按任务维度计算：`u_i × VG_si - v_i`。同一任务放不同显存规格的服务器，浪费差很多——80GB卡的浪费>24GB卡。Best-Fit只看GPU数量不管显存，在新公式下不是最优。

### 结果（100个case平均）

| 指标 | v5.0 | v5.1 | 变化 |
|------|------|------|------|
| E_wait | 112,236,844 | 112,237,548 | 持平 |
| E_memory | 40.15 | 27.87 | **-30.6%** |
| E_finish | 22,498 | 22,490 | 持平 |

E_memory大降三成，E_wait/E_finish不受影响。显存浪费最小化和GPU紧凑度是天然对齐的——浪费少的服务器往往GPU也更紧凑。

### 修改文件

- `src/scheduler.cpp`：tryStartOneJob 选服务器准则改为显存浪费优先
